...
# environ.setdefault("DJANGO_SETTINGS_MODULE", "WAD2_GROUPPROJECT.settings")
# django.setup()
# from lectureFinderApp.models import Course, Lecture, SavedLecture
...
def search_file_for_phrase(filename, phrase):
    ...
    for spoken_line in all_spoken_lines:
        ...
        spoken_line_words = spoken_line.split(" ")
        if phrase in spoken_line_words:
            return True
...
def search_transcripts_for_phrase(phrase):
    files_dir = "mock_data/transcripts"
    filenames = [f for f in listdir(files_dir) if f.endswith(".vtt")]

    for filename in filenames:
        print(filename)
        phraseAppearsInFile = search_file_for_phrase(f"{files_dir}/{filename}", phrase)

        filename_parts = filename.split("_")
        course_name = filename_parts[0]
        transcript_filename = filename_parts[1:]

        print(phraseAppearsInFile)


search_transcripts_for_phrase("algorithm")